...
const parsedResume = stringToHTMLparser.parseFromString(markdownToHTMLStringParser.makeHtml(resume), 'text/html');
...
for(let i = 0; i < article_length; i++) {
  ...
  if(article_link) {
    article_innerHTML += `<a href="${article_link.href}">${article_link.innerText}</a>`;
  }

  resume_articles[i].getElementsByTagName("main")[0].innerHTML += article_innerHTML;
}
